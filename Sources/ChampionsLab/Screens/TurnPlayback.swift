...
final class TurnPlayback: ObservableObject {
    ...
    func reset() {
        ...
        scene = nil
        stopTracks()
        damage = [:]; crits = []; boosts = [:]; abilities = [:]; partial = [:]; statusShown = [:]; pending = Withheld()
        struck = []; struckTheirs = []
    ...
    func play(_ steps: [Board.Step], hitMine: Set<Int>, hitTheirs: Set<Int>, singles: Bool,
        ...
        task = Task { @MainActor in
            ...
            for index in steps.indices where index > lastAction && steps[index].action == nil {
                ...
                guard !Task.isCancelled else { return }
                withAnimation(.easeIn(duration: 0.2)) { damage = [:]; crits = []; boosts = [:]; abilities = [:]; partial = [:]; statusShown = [:]; pending = Withheld() }
            }
